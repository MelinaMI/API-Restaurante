...
async function actualizarEstadoEnBackend(orderId, itemId, nuevoEstado) {
    try {
        ...
        console.warn("Error de conexión con el servidor, actualizando localmente.");
        actualizarEstadoLocalmente(orderId, itemId, nuevoEstado);
    }
}

function actualizarEstadoLocalmente(orderId, itemId, nuevoEstadoId) {
    const statusNames = {
        1: "Pending",
        2: "In progress",
        3: "Ready",
    ...
    const orderIndex = localOrders.findIndex(o => o.orderNumber == orderId);
    if (orderIndex !== -1) {
        const item = localOrders[orderIndex].items.find(i => i.id == itemId);
        if (item) {
            item.status.name = statusNames[nuevoEstadoId];
...
function fetchOrders() {
    fetch(`${API_URL}/Order`)
        ...
        .then(data => {
            ...
            ordersData = [...data, ...localOrders];
            renderAllOrders();
        })
        .catch(err => {
            console.warn("Usando órdenes locales (Mock Mode)");
            ordersData = JSON.parse(localStorage.getItem("localOrders") || "[]");
            renderAllOrders();
        });
}
