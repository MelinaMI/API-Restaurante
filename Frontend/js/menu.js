...
function fetchCategorias() {
    fetch(`${API_URL}/Category`)
        ...
        .catch(err => {
            console.warn("Usando categorías de respaldo (Mock)");
            categorias = window.mockCategorias || [];
            renderCategorias(categorias);
        });
}
...
function fetchPlatos() {
    fetch(`${API_URL}/Dish?onlyActive=true`)
        ...
        .catch(err => {
            console.warn("Usando platos de respaldo (Mock)");
            platos = window.mockPlatos || [];
            renderPlatos(platos);
        });
}
    ...
    orderForm.addEventListener("submit", async e => {
        ...
        try {
            ...
            try {
                ...
                console.warn("Error enviando a API, guardando localmente (Mock Mode)");
                // Simular respuesta exitosa para modo offline
                data = {
                    ...
                    items: pedido.items.map(item => {
                        ...
                        return {
                            ...
                            dish: { name: plato ? plato.name : "Plato" },
                            quantity: item.quantity,
                            notes: item.notes,
                            status: { name: "Pending" }
                        };
                    }),
                    deliveryType: { id: deliveryId },
                    deliverTo: deliveryTo,
                    notes: pedido.notes
                };

                // Guardar en orders locales para que aparezca en el panel
